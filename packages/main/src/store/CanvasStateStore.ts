...
export class CanvasStateStore extends Store<CanvasState> {
	...
	addObjects(...objects: Obj[]) {
		this.setState(
			this.state.setPage(
				new Transaction(this.state.page).insertObjects(objects).commit(),
			),
		);
	}
	...
	mergePoints(fromId: string, toId: string) {
		this.setState(
			this.state.setPage(
				new Transaction(this.state.page).mergePoints(fromId, toId).commit(),
			),
		);
	}
	...
	setTextAlign(textAlignX: TextAlignment, textAlignY: TextAlignment) {
		this.setState(
			this.state.setPage(
				new Transaction(this.state.page)
					.updateProperty(this.state.selectedObjectIds, (oldObject) => {
						switch (oldObject.type) {
							case "shape": {
								return { ...oldObject, textAlignX, textAlignY };
							}
							default: {
								return oldObject;
							}
			...
			case "select": {
				switch (this.state.dragType.type) {
					case "select": {
						...
						for (const obj of Object.values(this.state.page.objects)) {
							switch (obj.type) {
								case "shape": {
		...
		switch (this.state.mode) {
				...
				switch (dragType.type) {
					case "move-point": {
						...
						if (isNotNullish(nearestPoint?.pointId)) {
							...
							const line = this.state.page.objects[nearestPoint.lineId];
							assert(
								line !== undefined,
			...
			case "line": {
				...
				if (isNotNullish(nearestPoint1?.pointId)) {
					...
					const parentLine = this.state.page.objects[nearestPoint1.lineId];
					assert(
				...
				if (isNotNullish(nearestPoint2?.pointId)) {
					...
					const parentLine = this.state.page.objects[nearestPoint2.lineId];
					assert(
						parentLine !== undefined,
